...
@Data Used: Data used from Google Finance using panda datareader
...
# For Reading Stock Sata from GOOGLE
...
# What was the moving average of the Google?
moving_avg_days = [10,20,50]
GOOG['10days MA'] = GOOG['Close'].rolling(10).mean()
GOOG['20days MA'] = GOOG['Close'].rolling(20).mean()
GOOG['50days MA'] = GOOG['Close'].rolling(50).mean()

#table = plot(ff.create_table(GOOG.round(2).head(20), index = True, index_title = "Date"))

# Store the columns and plot table columns in a list
cols = ['Close', '10days MA', '20days MA', '50days MA']

fig_MA = plot(GOOG[cols].iplot(asFigure=True,title = "GOOGLE Moving Averages and Daily Price"))
# The Curves smooth out as we average over more days

### 2. Analyzing the Great Recession in Dec 2007 and Google price change 
df = DataReader("goog", 'google', datetime(2007, 10, 1), datetime(2010, 10, 1))
